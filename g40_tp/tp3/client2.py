    ...
    def run(self):
        ...
        while True:
            try:
                ...
                if message.lower() == "exit":  # Si l'utilisateur tape "exit", le client se déconnecte
                    self.send_message("Déconnexion")
                    break
                self.send_message(message)  # Envoi du message au serveur
